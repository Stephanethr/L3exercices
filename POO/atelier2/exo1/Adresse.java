...
/**
 * Classe Adresse servant à modéliser les informations contenues dans une
 * adresse
 * 
 * @author nivet_m
 *
...
public class Adresse {
	...
	public final void setCode_postal(String c_p) {
		if (code_postal.equals(INCONNU_CHAINE) && (c_p.matches("[0-9]{5}")))
			...
			if (!code_postal.equals(INCONNU_CHAINE))
				...
				System.err.println(
						"Code postal mal écrit, il doit être de la forme NNNNN avec N un chiffre différent de 0");
	...
	public final void setNumero(int n) {
		if (((numero == AUCUN_NUMERO) || (numero == INUTILE_NUMERO)) && (n > 0))
			numero = n;
		else
			System.err.println("Numero incorrect, il doit être positif...");
	}
	...
	public String toString() {
		...
		if ((numero != AUCUN_NUMERO) && (numero != INUTILE_NUMERO))
			result.append("n°").append(numero).append(",");
		if ((!rue.equals(INCONNU_CHAINE)) && (!(rue.equals(INUTILE_CHAINE))))
			result.append("rue ").append(rue);
		if (!code_postal.equals(INCONNU_CHAINE))
			result.append(" ").append(code_postal).append(" ");
		else
			result.append(" Code postal inconnu ");
		result.append(ville);
